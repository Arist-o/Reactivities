...
namespace API.Controllers.Report
{
    ...
    {
        [HttpGet]
        public async Task<ActionResult<List<ReportResponseDto>>> GetReport()
        {
            return HandleResult(await Mediator.Send(new GetReport.Query()));
        }

        [HttpPost]
        public async Task<ActionResult<ReportResponseDto>> createReport([FromBody] ReportCreateDto reportCreate)
        {
            return HandleResult(await Mediator.Send(new CreateReport.Command { reportCreateDto = reportCreate }));
        }

        [HttpPut]
        public async Task<ActionResult<ReportResponseDto>> EditReport([FromBody] ReportEditDto reportEdit)
        {
        ...
        public async Task<ActionResult<Unit>> DeleteReport(Guid Id)
        {
